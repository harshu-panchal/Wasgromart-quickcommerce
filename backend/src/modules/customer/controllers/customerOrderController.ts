    ...
    try {
        ...
        try {
            ...
            if (io) {
                ...
                const savedOrder = await Order.findById(newOrder._id).lean<any>();
                if (savedOrder) {
                    // For Online payments, notify sellers only after payment verification succeeds.
                    // COD orders can notify immediately.
                    const isCod = String(savedOrder.paymentMethod || '').toUpperCase() === 'COD';
                    const isPaid = String(savedOrder.paymentStatus || '').toUpperCase() === 'PAID';
                    if (isCod || isPaid) {
