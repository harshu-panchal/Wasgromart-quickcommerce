...
async function removeInvalidTokensFromUser(
  user: any,
  ...
  user.fcmTokens = (user.fcmTokens || []).filter(
    (token: string) => !invalidSet.has(token),
  );
  ...
  if (removedCount > 0) {
    // Determine user type by checking constructor name
    const userType = user.constructor.modelName; // Should be Delivery, Admin, Seller, or Customer
    if (userType === "Delivery") {
      await Delivery.findByIdAndUpdate(user._id, {
    ...
    console.log(
      `[${new Date().toISOString()}] Removed ${removedCount} invalid FCM token(s) for user ${user._id}`,
    );
  }
}

async function detachTokenFromOtherUsers(
  token: string,
  ...
  if (currentUserType !== "Customer") {
    operations.push(
      Customer.updateMany({ _id: { $ne: currentUserId } }, update),
    );
  } else {
    operations.push(
      Customer.updateMany({ _id: { $ne: currentUserId } }, update),
  ...
  if (currentUserType !== "Delivery") {
    operations.push(
      Delivery.updateMany({ _id: { $ne: currentUserId } }, update),
    );
  } else {
    operations.push(
      Delivery.updateMany({ _id: { $ne: currentUserId } }, update),
...
router.post("/save", authenticate, async (req: Request, res: Response) => {
  ...
  console.log(
    `[${timestamp}] User: ${req.user?.userId} (${req.user?.userType})`,
  );
  ...
  try {
    ...
    const rawToken =
      req.body.token || req.body.fcmToken || req.body.registrationToken;
    const token = typeof rawToken === "string" ? rawToken.trim() : "";

    if (!token) {
      console.warn(
        `[${new Date().toISOString()}] FCM POST /save - Missing token in body`,
      );
    ...
    const currentTokenList = isWebPlatform
      ? user.fcmTokens || []
      : user.fcmTokenMobile || [];
    ...
    if (isNewToken && now - lastNotified > cooldownMs) {
      ...
      try {
        ...
        await removeInvalidTokensFromUser(
          user,
          pushResponse?.invalidTokens || [],
        );
      ...
      if (!isNewToken) {
        console.log(
          `[${new Date().toISOString()}] Notification suppressed: Token already registered (re-registration)`,
        );
      } else {
        console.log(
...
router.post("/send-direct", async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `Sending direct test notification to: ${token} (Platform: ${platform || "unknown"})`,
    );
