...
export default function AdminAddProduct() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.seller) {
      setUploadError("Please select a seller. This is a required field.");
      return;
    }
    ...
    if (variations.length === 0) {
      setUploadError("Please add at least one product variation (e.g. Price and Stock).");
      return;
    }
    ...
    try {
      ...
      const productData: any = {
        ...
        // Fallbacks so backend validation passes
        price: variations[0]?.price || 0,
        stock: variations.reduce((acc, curr) => acc + (parseInt(curr.stock) || 0), 0),
      };
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold text-teal-800">Add Product</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-lg shadow-sm border border-neutral-200 overflow-hidden">
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-neutral-700 mb-2">Seller <span className="text-red-500">*</span></label>
                <select
                  name="seller"
              ...
              <div>
                <label className="block text-sm font-medium text-neutral-700 mb-2">Category <span className="text-red-500">*</span></label>
                <select
                  name="category"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={handleChange}
                  className="w-full px-4 py-2 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none bg-white font-normal"
                >
        ...
        <div className="bg-white rounded-lg shadow-sm border border-neutral-200 overflow-hidden">
          <div className="bg-teal-600 text-white px-6 py-4">
            <h2 className="text-lg font-semibold">Product Variations <span className="text-red-500">*</span></h2>
          </div>
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              <input
                ...
                className="px-4 py-2 border border-neutral-300 rounded-lg outline-none text-sm"
              />
              <input
                type="text"
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  ...
                  {variations.length === 0 && (
                    <tr>
                      <td colSpan={5} className="p-4 text-center text-neutral-400 text-sm">Please add at least one variation (Price and Stock) above.</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        {/* Status Messages */}
        {uploadError && <div className="p-4 bg-red-100 text-red-700 rounded-lg text-sm">{uploadError}</div>}
        {successMessage && <div className="p-4 bg-green-100 text-green-700 rounded-lg text-sm">{successMessage}</div>}
        ...
        <div className="flex justify-end gap-4">
          <button
            ...
            className="px-6 py-2 border border-neutral-300 rounded-lg hover:bg-neutral-50 transition-colors text-sm font-medium"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={uploading}
            className={`px-8 py-2 bg-teal-600 text-white rounded-lg font-semibold hover:bg-teal-700 transition-colors text-sm ${uploading ? "opacity-50 cursor-not-allowed" : ""
              }`}
          >
