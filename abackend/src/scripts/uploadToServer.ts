/**
 * Phase 2: Upload Images to Hostinger Server via SFTP
 * 
 * Reads cloudinary-map.json, connects to Hostinger via SSH/SFTP,
 * and uploads all Wasgromart images to the uploads/ directory.
 * 
 * Usage: npx ts-node src/scripts/uploadToServer.ts [--dry-run]
 * 
 * Prerequisites: npm install ssh2-sftp-client
 */

import fs from "fs";
import path from "path";

// Dynamic import for ssh2-sftp-client (ESM-style fallback for TS)
let SFTPClient: any;
...
// Hostinger SSH credentials (from .env comments)
const SSH_CONFIG = {
  host: "147.93.99.211",
  port: 65002,
  username: "u910031778",
...
// Remote base directory for uploads
...
async function main() {
  ...
  if (!fs.existsSync(MAP_FILE)) {
    ...
    console.error("   Run buildCloudinaryMap.ts first!");
    process.exit(1);
  }
  ...
  // Deduplicate — multiple URLs can point to the same file
  const uniqueFiles = new Map<string, MappingEntry>();
  ...
  if (isDryRun) {
    // Just print what would be uploaded
    const folderCounts: Record<string, number> = {};
    for (const entry of filesToUpload) {
      folderCounts[entry.serverFolder] =
        (folderCounts[entry.serverFolder] || 0) + 1;
    }
  ...
  try {
    ...
    console.error("❌ ssh2-sftp-client not installed!");
    console.error("   Run: npm install ssh2-sftp-client @types/ssh2-sftp-client");
    process.exit(1);
  }
  ...
  try {
    ...
    for (const entry of filesToUpload) {
      const remoteDir = path.posix.dirname(
        `${REMOTE_UPLOAD_DIR}/${entry.serverKey}`
      );
    ...
    for (const entry of filesToUpload) {
      ...
      process.stdout.write(
        `\r   [${pct}%] ${completed}/${filesToUpload.length} | ${elapsed}s | ${entry.serverKey.substring(0, 50)}`
      );

      try {
        // Check if file already exists on server (skip if same size)
  ...
  console.log(`   ⏭️  Skipped (already exists): ${report.skipped}`);
  ...
  console.log("\n✅ Phase 2 complete! Next: run migrateCloudinaryUrls.ts");
}
