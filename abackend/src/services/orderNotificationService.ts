...
export interface OrderNotificationState {
  ...
  timeoutId?: NodeJS.Timeout | null; // Track the current cascading timeout
}

export const notificationStates = new Map<string, OrderNotificationState>();

/**
 * Clear the response timer for an active notification state
 */
function clearNotificationTimeout(state: OrderNotificationState): void {
  if (state.timeoutId) {
...
export async function notifyDeliveryBoysOfNewOrder(
  ...
  try {
    ...
    // Sort so connected delivery boys are first, preserving distance order inside both groups
    const connectedBoys: string[] = [];
    const disconnectedBoys: string[] = [];
    for (const boyId of idStrings) {
      const room = io.sockets.adapter.rooms.get(`delivery-${boyId}`);
      if (room && room.size > 0) {
        connectedBoys.push(boyId);
      } else {
        disconnectedBoys.push(boyId);
      }
    }
    ...
    const state: OrderNotificationState = {
      orderId,
      allNearbyDeliveryBoyIds: prioritizedIds,
...
export async function notifyNextDeliveryBoy(
  ...
  // Fetch FCM push tokens for this delivery boy to see if we can notify them in background
  let tokens: string[] = [];
  let deliveryBoyData: any = null;
  try {
    deliveryBoyData = await Delivery.findById(nextDeliveryBoyId)
      .select("fcmTokens fcmTokenMobile")
      .lean();
    tokens = [
      ...(deliveryBoyData?.fcmTokens || []),
      ...(deliveryBoyData?.fcmTokenMobile || []),
    ].filter(Boolean);
  } catch (dbErr) {
    console.error(`Failed to fetch delivery boy ${nextDeliveryBoyId} data:`, dbErr);
  }
  ...
  // 2. Send Firebase push notification if tokens are available (to wake up background app)
  if (tokens.length > 0) {
    try {
      ...
      console.error(
        `Failed to send push notification to delivery boy ${nextDeliveryBoyId}:`,
        pushErr,
      );
    }
  }
...
export async function handleOrderRejection(
  ...
  try {
    ...
    if (!state) {
      return {
        success: false,
        message: "Order notification not found",
        allRejected: false,
      };
    }
    ...
    if (!state.notifiedDeliveryBoys.has(normalizedDeliveryBoyId)) {
      console.warn(
        `⚠️ Delivery boy ${normalizedDeliveryBoyId} not in notified list for order ${orderId}.`,
      );
      return {
        success: false,
        message: "You were not notified about this order",
    ...
    // Clear the active timer when manually rejected so we don't trigger dual cascades
    clearNotificationTimeout(state);
