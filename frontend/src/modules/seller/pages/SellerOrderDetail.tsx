...
export default function SellerOrderDetail() {
  ...
  const handleExportPDF = () => {
    ...
    doc.text(sellerProfile?.storeName || "Store", margin + 5, yPos + 10);
    ...
    doc.setFontSize(12);
    doc.setFont("helvetica", "bold");
    doc.text(sellerProfile?.storeName || "Store", margin, yPos);
    yPos += 7;

    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`From: ${sellerProfile?.storeName || "Store"}`, margin, yPos);
    yPos += 6;
    doc.text(`Phone: ${sellerProfile?.mobile || "—"}`, margin, yPos);
    yPos += 6;
    doc.text(`Email: ${sellerProfile?.email || "—"}`, margin, yPos);
    yPos += 6;
    if (sellerProfile?.address) {
      doc.text(`Address: ${sellerProfile.address}${sellerProfile.city ? `, ${sellerProfile.city}` : ""}`, margin, yPos);
      yPos += 6;
    }
    yPos += 6;
    ...
    doc.text(`Bill Generated by ${sellerProfile?.storeName || "Store"}`, pageWidth / 2, yPos, {
      align: "center",
    });
    yPos += 8;
    ...
    doc.setFontSize(8);
    doc.text(
      `Copyright © ${new Date().getFullYear()}. ${sellerProfile?.storeName || "Store"}`,
      pageWidth / 2,
      yPos,
      { align: "center" }
    );
          ...
          <div className="flex flex-col lg:flex-row justify-between gap-6 mb-6">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-8 h-8 bg-teal-700 rounded flex items-center justify-center">
                  <span className="text-white text-xs font-bold">
                    {(sellerProfile?.storeName || "S").charAt(0).toUpperCase()}
                  </span>
                </div>
                <div>
              ...
              <div className="text-sm text-neutral-600 mb-1">
                <span className="font-medium">From:</span> {sellerProfile?.storeName || "—"}
              </div>
              <div className="text-sm text-neutral-600 space-y-1">
                <div>
                  <span className="font-medium">Phone:</span> {sellerProfile?.mobile || "—"}
                </div>
                <div>
                ...
                {sellerProfile?.address && (
                  <div>
                    <span className="font-medium">Address:</span> {sellerProfile.address}{sellerProfile.city ? `, ${sellerProfile.city}` : ""}
                  </div>
                )}
              </div>
            </div>
